...
class KalmanFilterXYAH:
    ...
    def multi_step_look_ahead(self, mean: np.ndarray, steps=[5, 10, 15, 20]):
        """
        Predict future positions (mean only) at multiple time steps.

        Args:
            mean (np.ndarray): 1x8 mean vector of the current object state
            steps (tuple): Future steps to predict

        Returns:
            list: [(x1, y1), (x5, y5), ..., (x20, y20)] predicted positions
        """
        F = self._motion_mat
