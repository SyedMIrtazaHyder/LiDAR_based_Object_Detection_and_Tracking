...
class BYTETracker:
    ...
    def multi_step_look_ahead(self, steps=[5, 10, 15, 20]):
        """Predicts the future positions of the tracked objects using a multi-step look-ahead approach."""
        if len(self.tracked_stracks) <= 0:
            return
        multi_mean = np.asarray([st.mean.copy() for st in self.tracked_stracks])
        traj = STrack.shared_kalman.multi_step_look_ahead(multi_mean, steps)
        return traj      
